		...
		sc_form.each(function() {
			...
			if (js__datepicker.length > 0) {
				js__datepicker.pickadate({
					...
					format: 'http://hotcoffee-html.themerex.net/js/custom/dd.mm.yyyy',
...
function hotcoffee_sc_change_active_pagination_in_slider(slider, idx) {
	...
	if (off < 0) {
		sc_scroll_wrapper.css({'transform': 'translate3d(0px, 0px, 0px)', 'transition-duration': 'http://hotcoffee-html.themerex.net/js/custom/0.3s'});
	} else if (h <= off+h2) {
		sc_scroll_wrapper.css({'transform': 'translate3d(0px, -'+(Math.abs(off2)+off-h/4)+'px, 0px)', 'transition-duration': 'http://hotcoffee-html.themerex.net/js/custom/0.3s'});
	}
}
